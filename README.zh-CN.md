...
| 软件          | Neovim（LazyVim）、Pi coding agent、mise、lazygit、btop、fastfetch、yazi                             |
| 增强 · 提示符 | Starship、tealdeer、markdownlint-cli2                                                                |
| 增强 · 命令行 | ripgrep、fd、bat、glow、eza、dua-cli、gum、tree-sitter-cli、delta、trash-cli、pay-respects、gitleaks |
...
Pi 的公共指令、权限、提示词、插件参数和精确锁定的插件清单位于 `config/pi/`。
`dotman` 会把它们链接到 `~/.pi`，并在 `~/.local/bin` 暴露
`pi-plugin-stack`。

插件安装是可选且幂等的部署步骤：仅当 `pi` 和 `jq` 均可用，且已安装版本或
`settings.json.packages` 与 `config/pi/plugins.json` 不一致时执行。也可以手动运行：
...
清单只接受精确的 npm 版本。安装时会保留 `packages` 之外的所有机器本地设置；
任一插件安装失败时会恢复之前的 `settings.json`。

全局权限策略允许文件编辑以及常见的只读 Shell、构建、lint 和测试命令直接运行。
未知命令、Shell 组合与重定向、原地转换、部署、发布及其他潜在危险操作仍需确认。

Web Access 启用了 `librarian` 技能，可以克隆公开仓库，以源码证据调查依赖；
浏览器 Cookie 访问仍保持关闭。Pi Lens 使用共享的 `config/pi-lens/config.json`，
忽略生成目录和依赖目录、报告可操作警告、延迟到回合末格式化，并且不自动应用修复。

真实的 `~/.pi/agent/settings.json` 保持机器本地。安装器只同步其中的 `packages`
字段，不覆盖 provider、model 或其他偏好。`models.json`、`auth.json`、API Key、
信任决策、会话、缓存和 MCP 服务定义均不由本仓库管理。
